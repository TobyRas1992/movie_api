...
let allowedOrigins = ['http://localhost:8080', 'http://testsite.com', 'http://localhost:1234']; // specifies allowed websites for CORS.
...
app.post('/movies', [check('Title', 'Title is required.').not().isEmpty(), check('Description', 'Description is required').not().isEmpty(), check('Actors', 'Actors need to be in an array.').isArray(), check('ImagePath', 'Correct image path required').isLength({min: 5}), check('Featured', 'Boolean value required.').isBoolean(), check('Released', 'Release year required').isLength({min: 4})], (req, res) =>{
